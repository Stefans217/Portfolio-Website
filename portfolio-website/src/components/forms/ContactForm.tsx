...
export default function ContactForm() {
  ...
  const hasError = (name: keyof ContactPayload) =>
    Boolean(touched[name] && clientErrors[name]);
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        setSubmit({
          ...
          fieldErrors: "fieldErrors" in data ? data.fieldErrors : undefined,
        });
        return;
      }
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FormField
          ...
          onChange={onChange}
          onBlur={onBlur}
          error={hasError("name") ? clientErrors.name : undefined}
        />
        <FormField
          label="Email"
          name="email"
          ...
          onChange={onChange}
          onBlur={onBlur}
          error={hasError("email") ? clientErrors.email : undefined}
        />
      </div>
      ...
      <FormTextArea
        ...
        onChange={onChange}
        onBlur={onBlur}
        error={hasError("message") ? clientErrors.message : undefined}
        rows={6}
      ...
      {submit.status === "success" && (
        <div
          className="rounded-md border border-emerald-800 bg-emerald-950/30 p-3 text-sm text-emerald-300"
          role="status"
        >
          {submit.message}
        </div>
      ...
      <button
        ...
        className="inline-flex w-full items-center justify-center rounded-md bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/40 disabled:cursor-not-allowed disabled:opacity-50"
      >
  ...
  return (
    <div>
      ...
      <input
        ...
        autoComplete={
          name === "email" ? "email" : name === "name" ? "name" : "on"
        }
      />
      {error ? (
  ...
  return (
    <div>
      ...
      <textarea
        ...
        aria-invalid={Boolean(error)}
        aria-describedby={describedBy}
        className="block w-full resize-y rounded-md border border-white/10 bg-white/10 px-3 py-2 text-sm text-white placeholder-gray-500 outline-none focus:border-white/30 focus:ring-0"
